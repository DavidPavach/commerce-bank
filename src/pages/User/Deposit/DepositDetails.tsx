...
//Icons
import { Copy, MoneyRecive, Check, Tether } from "iconsax-react";
import { AlertTriangle, CheckCircle, Clock, Edit3, Loader, Save, X } from "lucide-react";

const getStatusBadge = (status: string) => {
    switch (status) {
        case "successful":
            return (
                <Badge className="flex items-center gap-x-1 bg-green-100 hover:bg-green-100 text-green-800">
        ...
        case "pending":
            return (
                <Badge className="flex items-center gap-x-1 bg-yellow-100 hover:bg-yellow-100 text-yellow-800">
        ...
        case "failed":
            return (
                <Badge className="flex items-center gap-x-1 bg-red-100 hover:bg-red-100 text-red-800">
                    <X className="size-3" />
                    <span>Failed</span>
                </Badge>
            )
...
const getAcceptanceBadge = (isAccepted: string) => {
    switch (isAccepted) {
        case "accepted":
            return (
                <Badge className="flex items-center gap-x-1 bg-blue-100 hover:bg-blue-100 text-blue-800">
                    <CheckCircle className="size-3" />
                    <span>Accepted</span>
                </Badge>
            )
        case "pending":
            return (
                <Badge className="flex items-center gap-x-1 bg-orange-100 hover:bg-orange-100 text-orange-800">
                    <Clock className="size-3" />
...
const DepositDetails = ({ deposit }: { deposit: Deposits }) => {
    ...
    const toggleEditing = () => setEditingHash((prev) => !prev)
    const toggleShowWallet = () => setShowWallet((prev) => !prev);
    ...
    return (
        <>
            {showWallet && <WalletDisplay onClose={toggleShowWallet} />}
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center">
                                            <h3 className="font-bold text-slate-900 text-base md:text-lg xl:text-xl">{formatCurrency(deposit.amount)}</h3>
                                            {deposit.isAccepted === "accepted" && (
                                                <Tether variant="Bold" onClick={toggleShowWallet} className="size-7 md:size-8 xl:size-9 text-primary cursor-pointer" />
                                            )}
                                        </div>
                                        <p className="text-[11px] text-slate-600 md:text-xs xl:text-sm">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {copied === `hash-${deposit.hash}` ? (
                                                ...
                                                <Copy onClick={() => copyToClipboard(deposit.hash, "hash")} className="size-3" />
                                            )}
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {editingHash ? (
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <Button size="sm" onClick={handleUpdateHash} disabled={!editHash} className="h-8 text-white" style={{ backgroundColor: "#1D9B5E" }}>
                                                    {updateHash.isPending ? <Loader className="size-3 animate-spin" /> : <Save className="size-3" />}
                                                </Button>
                                                <Button size="sm" variant="outline" className="h-8" onClick={toggleEditing}>
                                                    <X className="size-3" />
                                                </Button>
                                            </div>
                                        ) : (
                                            <Button variant="outline" onClick={toggleEditing}>
                                                <Edit3 className="mr-1 size-3" /> Enter Transaction Hash
                                            </Button>
                                        )}
                                    </div>
                                )}
